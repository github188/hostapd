...
enum { RADIUS_ATTR_USER_NAME = 1,
       ...
       RADIUS_ATTR_FILTER_ID = 11,/*add for qos lever*/
       ...
       RADIUS_ATTR_USER_DATA_QOS = 220	/*私有TAG：擒带用户签约信息*/
};
...
/* NAS-Port-Type */
#define RADIUS_NAS_PORT_TYPE_ETHERNET	5
...
#define RADIUS_VENDOR_ID_ASCEND	529

enum{
	RADIUS_VENDOR_ATTR_ASCEND_DATA_RATE	= 197,
	RADIUS_VENDOR_ATTR_ASCEND_XMIT_RATE	= 255
};
...
enum{
	RADIUS_VENDOR_ATTR_WISPR_BANDWIDTH_MIN_UP		= 5,
	RADIUS_VENDOR_ATTR_WISPR_BANDWIDTH_MIN_DOWN	= 6,
	RADIUS_VENDOR_ATTR_WISPR_BANDWIDTH_MAX_UP		= 7,
	RADIUS_VENDOR_ATTR_WISPR_BANDWIDTH_MAX_DOWN	= 8
};


#define RADIUS_VENDOR_ID_MIKROTIK 14988

enum{
	RADIUS_VENDOR_ATTR_MIKROTIK_RATE_LIMIT		= 8
};


#define RADIUS_VENDOR_ID_ZTE	3902

enum{
	RADIUS_VENDOR_ATTR_ZTE_RATE_CTRL_SRC_DOWN = 83,	//u_int32_t, kbps
	RADIUS_VENDOR_ATTR_ZTE_RATE_CTRL_SRC_UP	= 89		//u_int32_t, kbps
};
...
{
    /* 内网QOS */
    unsigned int    ulInPriority;   /* 优先级 */
    unsigned int    ulInMaxULBand;  /* 最大上行带宽，单位: Kbps */
    unsigned int    ulInMaxDLBand;  /* 最大下行带宽，单位：Kbps */
    unsigned int    ulInMinULBand;  /* 最低上行保证带宽，单位: Kbps */
    unsigned int    ulInMinDLBand;  /* 最低上行保证带宽，单位: Kbps */

    /* 外网QOS */
    unsigned int    ulOutPriority;  /* 优先级 */
    unsigned int    ulOutMaxULBand; /* 最大上行带宽，单位: Kbps */
    unsigned int    ulOutMaxDLBand; /* 最大下行带宽，单位：Kbps */
    unsigned int    ulOutMinULBand; /* 最低上行保证带宽，单位: Kbps */
    unsigned int    ulOutMinDLBand; /* 最低上行保证带宽，单位: Kbps */
}AC_USER_DATA_QOS_T;
...
#define RADIUS_802_1X_ADDR_LITTLE_FORMAT  "%02x-%02x-%02x-%02x-%02x-%02x"

#define RADIUS_802_1X_ADDR_OEM_FORMAT          "%02X:%02X:%02X:%02X:%02X:%02X"  
#define RADIUS_802_1X_ADDR_OEM_LITTLE_FORMAT   "%02x:%02x:%02x:%02x:%02x:%02x"
