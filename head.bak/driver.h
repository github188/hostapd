...
struct driver_ops {
	...
	int (* set_authmode)(const char *ifname, void *priv, int authmode) ;
...
static inline int 
hostapd_set_authmode(const char *ifname, struct hostapd_data *hapd,
		      int authmode)
{
	if(hapd->driver == NULL || hapd->driver->set_authmode == NULL)
		return 0;
	return hapd->driver->set_authmode(ifname, hapd->driver, authmode);
}
