...
extern int hostapd_inet_aton(const char *cp, u_int32_t * addr);
#if 0
...
{
	bss->logger_syslog_level = HOSTAPD_LEVEL_DEBUG_VERBOSE;
	bss->logger_stdout_level = HOSTAPD_LEVEL_DEBUG_VERBOSE;
	bss->logger_syslog = (unsigned int) -1;
	bss->logger_stdout = (unsigned int) -1;
	bss->debug = HOSTAPD_DEBUG_EXCESSIVE ;

	bss->auth_algs = HOSTAPD_AUTH_OPEN ;
...
{
	if (hostapd_inet_aton(txt, (u_int32_t *)&addr->u.v4)) {
		addr->af = AF_INET;
		return 0;
	}
				       ...
				       int *num)
{
	...
	while (fgets(buf, sizeof(buf), f)) {
		...
		newacl = (macaddr *) VM_REALLOC(*acl, (*num + 1) * ETH_ALEN);
		if (newacl == NULL) {
					...
					struct hostapd_bss_config *conf)
{
	...
	while (fgets(buf, sizeof(buf), f)) {
		...
		if (buf[0] == '*') {
			...
			user->identity = VM_MALLOC(pos - start);
		...
		if (*pos == '"') {
			...
			user->password = VM_MALLOC(pos - start);
			...
			user->password = VM_MALLOC((pos2 - pos) / 2);
			if (user->password == NULL) {
				printf("Failed to allocate memory for EAP "
				       "password\n");
				goto failed;
	...
	failed:
		if (user) {
			VM_FREE(user->password);
			VM_FREE(user->identity);
			VM_FREE(user);
		}
		ret = -1;
		break;
				...
				struct hostapd_radius_server **curr_serv)
{
	...
	nserv = VM_REALLOC(*server, (*num_server + 1) * sizeof(*nserv));
	if (nserv == NULL)
		return -1;
...
#endif
int hostapd_config_check_bss(struct hostapd_bss_config *bss,
				    struct hostapd_config *conf)
{
...
int hostapd_config_bss_read_wep(struct hostapd_wep_keys *wep, int keyidx,
				   char *val, u32 format)
{
	size_t len = strlen(val);
	...
	if (format == 2) {/*ASCIIÂë*/
		...
		if (wep->key[keyidx] == NULL)
			return -1;
		memcpy(wep->key[keyidx], val, len);
		wep->len[keyidx] = len;
	} else if(format == 1){/*Ê®Áù½øÖÆ×Ö·û´®*/
		if (len != 10 && len != 26)
			return -1;
		len /= 2;
		wep->key[keyidx] = VM_MALLOC(len);
...
static int hostapd_parse_rates(int **rate_list, char *val)
{
	...
	*rate_list = NULL;
	...
	list = VM_MALLOC(sizeof(int) * (count + 2));
	if (list == NULL)
		return -1;
	pos = val;
	count = 0;
...
{
	printf("%s, null function, FUNC(VapLoadConfig) can be stead of it.\n", __func__) ;
	struct hostapd_bss_config *bss;

	if (*ifname == '\0')
		return -1;

	bss = VM_REALLOC(conf->bss, (conf->num_bss + 1) *
...
struct hostapd_config * hostapd_config_read(const char *fname)
{
	...
	while (fgets(buf, sizeof(buf), f)) {
		...
		*pos = '\0';
		...
		if (strcmp(buf, "interface") == 0) {
			...
			VM_FREE(bss->radius_server_clients);
	...
	return conf;
#else
	printf("%s, this function is invalid, configure is read from DB now.\n", __func__) ;
	return NULL ;
#endif
}
				       ...
				       int num_servers)
{
	...
	for (i = 0; i < num_servers; i++) {
		VM_FREE(servers[i].shared_secret);
	}
	VM_FREE(servers);
}
#endif
...
void hostapd_config_free_bss(struct hostapd_bss_config *conf)
{
	...
	VM_FREE(conf->ssid.wpa_passphrase);
	VM_FREE(conf->ssid.wpa_psk_file);
#ifdef CONFIG_FULL_DYNAMIC_VLAN
	VM_FREE(conf->ssid.vlan_tagged_interface);
#endif /* CONFIG_FULL_DYNAMIC_VLAN */
	...
	VM_FREE(conf->dump_log_name);
	VM_FREE(conf->eap_req_id_text);
	VM_FREE(conf->accept_mac);
	VM_FREE(conf->deny_mac);
	VM_FREE(conf->nas_identifier);
#if 0
...
#endif
	VM_FREE(conf->rsn_preauth_interfaces);
	VM_FREE(conf->ctrl_interface);
	VM_FREE(conf->ca_cert);
	VM_FREE(conf->server_cert);
	VM_FREE(conf->private_key);
	VM_FREE(conf->private_key_passwd);
	VM_FREE(conf->eap_sim_db);
	VM_FREE(conf->radius_server_clients);
	VM_FREE(conf->test_socket);
#if 0
	VM_FREE(conf->radius);
#endif
	...
	if (conf->ssid.dyn_vlan_keys) {
		...
		for (i = 0; i <= ssid->max_dyn_vlan_keys; i++) {
			...
			VM_FREE(ssid->dyn_vlan_keys[i]);
		}
		VM_FREE(ssid->dyn_vlan_keys);
		ssid->dyn_vlan_keys = NULL;
	}
	memset(conf, 0, sizeof(struct hostapd_bss_config)) ;
}
