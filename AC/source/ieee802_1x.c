...
ieee802_1x_group_alloc(struct hostapd_data *hapd, const char *ifname)
{
	...
	if (!key->key[key->idx])
		key->key[key->idx] = VM_MALLOC(key->default_len);
		     ...
		     size_t vlan_id)
{
	...
	if (ssid->max_dyn_vlan_keys < vlan_id) {
		...
		int size = (vlan_id + 1) * sizeof(ssid->dyn_vlan_keys[0]);
		na = VM_REALLOC(ssid->dyn_vlan_keys, size);
		if (na == NULL)
			return NULL;
...
void ieee802_1x_tx_key(struct hostapd_data *hapd, struct sta_info *sta)
{
	...
	if (hapd->conf->individual_wep_key_len > 0) {
		u8 *ikey;
		ikey = VM_MALLOC(hapd->conf->individual_wep_key_len);
		if (ikey == NULL ||
					  ...
					  u8 *eap, size_t len)
{
...
#else
	sm->radius_identifier = radius_client_get_id(sta->radius);
#endif
	...
	if (sta->nas_id &&
	    !radius_msg_add_attr(msg, RADIUS_ATTR_NAS_IDENTIFIER,
				 sta->nas_id, strlen((char *)sta->nas_id))) {
		printf("Could not add NAS-Identifier\n");
		goto fail;
	}
#endif
	...
	snprintf(buf, sizeof(buf), "%u", sta->aid) ;
...
#else
	radius_client_send(sta->radius, msg, RADIUS_AUTH, sta->addr);
#endif
	return;
				...
				u8 *data, size_t len)
{
	...
	VM_FREE(sm->last_eap_supp);
	sm->last_eap_supp_len = sizeof(*eap) + len;
	sm->last_eap_supp = (u8 *) VM_MALLOC(sm->last_eap_supp_len);
				 ...
				 struct radius_class_data *src)
{
	...
	for (i = 0; i < src->count; i++) {
		dst->attr[i].data = VM_MALLOC(src->attr[i].len);
...
void ieee802_1x_free_station(struct sta_info *sta)
{
	...
	VM_FREE(sm->eapol_key_sign);
	VM_FREE(sm->eapol_key_crypt);
	eapol_sm_free(sm);
}
					  ...
					  struct sta_info *sta)
{
	...
	case EAP_CODE_SUCCESS:
		...
		break;
	case EAP_CODE_FAILURE:{
		AC_OMA_EVENT_EXT_INFO_U ext = {0} ;
			
		snprintf(buf, sizeof(buf), "EAP Failure");

		os_memcpy(ext.stUser.aucMac, sta->addr, 6) ;
		if(hostapd_report_event(hapd, 1, AC_OMA_EVENT_ID_USER_ACCESS_DENY, AC_OMA_EVENT_REASON_USER_ACCESS_DENY_8021X, & ext) == 0)
			hostapd_logger(hapd, sta->addr, HOSTAPD_MODULE_IEEE8021X, HOSTAPD_LEVEL_DEBUG_VERBOSE,
				"%s, report event success[illegal user access(%d/%d/%d) failed: 8021X failed].\n", 
			...
			hostapd_logger(hapd, sta->addr, HOSTAPD_MODULE_IEEE8021X, HOSTAPD_LEVEL_WARNING,
				"%s, report event failed[illegal user access(%d/%d/%d) failed: 8021X failed].\n", 
				__func__, hapd->iface->pstWtp->usWtpId, hapd->iface->interface_id, hapd->vap_id) ;
		break;
	}
	default:
					  ...
					  struct radius_msg *msg)
{
	...
	for (i = 0; i < count; i++) {
		...
		nclass[nclass_count].data = VM_MALLOC(class_len);
					   ...
					   struct radius_msg *msg)
{
	...
	sta->identity = (u8 *)os_strdup((char *)identity) ;
}


#if 0
/* Update user info based on USER_DATA_QOS attribute in Access-Accept */
static void ieee802_1x_update_sta_info(struct hostapd_data * hapd, struct sta_info * sta, struct radius_msg * msg)
{
	...
	len = radius_msg_get_attr(msg, RADIUS_ATTR_USER_DATA_QOS, & user_info, sizeof(user_info)) ;
	if(len == sizeof(user_info)){
		...
		sta->user_qos.ulPriority = user_info.ulOutPriority ;
		sta->user_qos.ulUpBandMax = user_info.ulOutMaxULBand ;
		sta->user_qos.ulUpBandMin = user_info.ulOutMinULBand ;
		sta->user_qos.ulDownBandMax = user_info.ulOutMaxDLBand ;
		sta->user_qos.ulDownBandMin = user_info.ulOutMinDLBand ;
	}
			...
			void *data)
{
	...
	struct sta_info *sta;
	u32 session_timeout = 0, termination_action, acct_interim_interval, acct_idle_timeout;
    ...
    /*rfc 2865, attr 27 :session_timeout*/
	session_timeout_set =
		!radius_msg_get_attr_int32(msg, RADIUS_ATTR_SESSION_TIMEOUT,
					   &session_timeout);
    if(session_timeout > 0)
    {
        sta->acct_session_timeout = session_timeout;
    }

    /*rfc 2865, attr 28 : idle_timeout*/
   	if (msg->hdr->code == RADIUS_CODE_ACCESS_ACCEPT)
    {
    	if(!radius_msg_get_attr_int32(msg, RADIUS_ATTR_IDLE_TIMEOUT, &acct_idle_timeout) && acct_idle_timeout > 0)
        {
            sta->acct_idle_time = acct_idle_timeout;
        }
    }
    
    /*rfc 2865, attr 29 :Termination-Action*/
	...
	HOSTAPD_DEBUG(HOSTAPD_DEBUG_MINIMAL,
		      "%s, RADIUS response : idletime :%d,interim_interval :%d,session_timeout :%d,Termination_action:%d\n",
		      __func__,sta->acct_idle_time,sta->acct_interim_interval,sta->acct_session_timeout,termination_action);
	...
	case RADIUS_CODE_ACCESS_ACCEPT:
...
#if 0		
		ieee802_1x_update_sta_info(hapd, sta, msg) ;
#else
		/*begin by fangz for 802.1x*/
		sta->vlan_id = radius_msg_get_vlanid(msg);
		if(sta->vlan_id == -1)
		{
			ModuleLogMsg(APP_MODULE_HOSTAPD, APP_WARN,
					"%s,get 802.1x vlan fail.\n", 
					__func__) ;			
			sta->vlan_id = 0;
		}
		/*end by fangz for 802.1x*/
		radius_msg_get_qos_info(msg, &sta->user_qos);
		...
		if(radius_msg_get_attr(msg, RADIUS_ATTR_FILTER_ID, acFilterId, sizeof(acFilterId)) > 0)
		{
			ModuleLogMsg(APP_MODULE_HOSTAPD, APP_WARN,
					"%s,Filter id :%s.\n", 
					__func__, acFilterId) ;
			strncpy(sta->acUserGroupName, acFilterId, strlen(acFilterId));
		}
...
static int ieee802_1x_rekey_broadcast(struct hostapd_data *hapd)
{
	...
	hapd->default_wep_key = VM_MALLOC(hapd->conf->default_wep_key_len);
