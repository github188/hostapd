...
static void eapol_sm_set_eapReqData(void *ctx, const u8 *eapReqData, size_t eapReqDataLen);
static void eapol_sm_set_eapKeyData(void *ctx, const u8 *eapKeyData, size_t eapKeyDataLen);
...
SM_STATE(AUTH_PAE, RESTART)
{
	if (sm->auth_pae_state == AUTH_PAE_AUTHENTICATED) {
		if (sm->reAuthenticate){
			...
			sm->hapd->stats.auth_8021x_req_reauth ++ ;
		}
		if (sm->eapolStart)
	...
	sm->hapd->stats.auth_8021x_req ++ ;
	//g_stStats.ulRadiusAuthReq2++;     /* 2011.4.6 */
	sm->eapRestart = TRUE;
...
SM_STEP(AUTH_PAE)
{
	if ((sm->portControl == Auto && sm->portMode != sm->portControl) ||
		...
		case AUTH_PAE_CONNECTING:
			...
			if (sm->eapolLogoff)
				SM_ENTER(AUTH_PAE, DISCONNECTED);
			if (sm->reAuthCount > sm->reAuthMax){
				SM_ENTER(AUTH_PAE, DISCONNECTED);
				ap_sta_session_timeout(sm->hapd, sm->sta, 0) ;
			}
#endif
			else if ((sm->eapReq &&
...
SM_STATE(BE_AUTH, SUCCESS)
{
	...
	sm->authSuccess = TRUE;
	sm->keyRun = TRUE;
	sm->hapd->stats.auth_8021x_success ++ ;
}
				    ...
				    size_t eapReqDataLen)
{
	...
	sm->last_eap_radius = VM_MALLOC(eapReqDataLen);
	if (sm->last_eap_radius == NULL)
		return;
				 ...
				 struct eap_user *user)
{
	...
	if (eap_user->password) {
		user->password = VM_MALLOC(eap_user->password_len);
