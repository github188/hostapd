...
static struct radius_attr_type radius_attrs[] =
{
	...
	{ RADIUS_ATTR_EVENT_TIMESTAMP, "Event-Timestamp",
	  RADIUS_ATTR_INT32 },
	{ RADIUS_ATTR_CHAP_CHALLENGE, "Chap-Challenge", RADIUS_ATTR_UNDIST },
...
static void radius_msg_dump_attr(struct radius_attr_hdr *hdr)
{
	...
	if (attr == NULL){
		ModuleLogMsg(APP_MODULE_HOSTAPD, APP_DETAIL, "%s",buf) ;
		return;
	}
	...
	switch (attr->data_type) {
	case RADIUS_ATTR_TEXT:
		snprintf(buf+os_strlen(buf), sizeof(buf)-os_strlen(buf), "      Value: '") ;
		for (i = 0; i < len; i++){
			if (pos[i] >= 32 && pos[i] < 127)
				snprintf(buf+os_strlen(buf), sizeof(buf)-os_strlen(buf), "%c", pos[i]) ;
			else
				snprintf(buf+os_strlen(buf), sizeof(buf)-os_strlen(buf), "<%02x>", pos[i]) ;
		}
		snprintf(buf+os_strlen(buf), sizeof(buf)-os_strlen(buf), "'\n") ;
		break;
	...
	case RADIUS_ATTR_IP:
		if (len == 4) {
			struct in_addr addr;
			os_memcpy(&addr, pos, 4);
			snprintf(buf+os_strlen(buf), sizeof(buf)-os_strlen(buf), "      Value: %s\n", inet_ntoa(addr)) ;
		} else
			snprintf(buf+os_strlen(buf), sizeof(buf)-os_strlen(buf), "      Invalid IP address length %d\n", len) ;
		break;
	...
	case RADIUS_ATTR_IPV6:
		if (len == 16) {
			...
			snprintf(buf+os_strlen(buf), sizeof(buf)-os_strlen(buf), "      Invalid IPv6 address length %d\n", len) ;
		break;
#endif /* CONFIG_IPV6 */
	...
	case RADIUS_ATTR_INT32:
		if (len == 4)
			snprintf(buf+os_strlen(buf), sizeof(buf)-os_strlen(buf), "      Value: %u\n", WPA_GET_BE32(pos)) ;
		else
			snprintf(buf+os_strlen(buf), sizeof(buf)-os_strlen(buf), "      Invalid INT32 length %d\n", len) ;
		break;
...
/* Update user info */
void radius_msg_get_qos_info(struct radius_msg * msg, AC_USR_QOS_T * pstQos)
{
	AC_USER_DATA_QOS_T user_info = {0} ;
	u_int32_t ulRate = 0, *pulRate = NULL, *pulRateMinUp = NULL, *pulRateMinDown = NULL, *pulRateMaxUp = NULL, *pulRateMaxDown = NULL, *pulRateUp = NULL, *pulRateDown = NULL;
	u_int8_t *pszRateLimit = NULL, class_rate[33] = {0};
	int len, attr_rate = 0 ;
	...
	/*ASCEND*/
	pulRate = (u_int32_t *)radius_msg_get_vendor_attr(msg, RADIUS_VENDOR_ID_ASCEND, RADIUS_VENDOR_ATTR_ASCEND_DATA_RATE, (size_t *)&len);
	if(pulRate && len == 4){
		ulRate = ntohl(*pulRate);
		VM_FREE(pulRate);
		pulRate = NULL;

		ModuleLogMsg(APP_MODULE_HOSTAPD, APP_DETAIL, "ascend attr(data rate): %d", ulRate);
		pstQos->ulPriority = 0;
		pstQos->ulUpBandMax = pstQos->ulUpBandMin = pstQos->ulDownBandMax = pstQos->ulDownBandMin = ulRate/1000;

		pulRate = (u_int32_t *)radius_msg_get_vendor_attr(msg, RADIUS_VENDOR_ID_ASCEND, RADIUS_VENDOR_ATTR_ASCEND_XMIT_RATE, (size_t *)&len);
		if(pulRate && len == 4){
			ulRate = ntohl(*pulRate);
			VM_FREE(pulRate);
			pulRate = NULL;

			ModuleLogMsg(APP_MODULE_HOSTAPD, APP_DETAIL, "ascend attr(xmit rate): %d", ulRate);
			pstQos->ulDownBandMax = pstQos->ulDownBandMin = ulRate/1000;
		}
		else if(pulRate){
			ModuleLogMsg(APP_MODULE_HOSTAPD, APP_ERROR, "error len: ascend attr(xmit rate): %d", len);
		...
		ModuleLogMsg(APP_MODULE_HOSTAPD, APP_ERROR, "error len: ascend attr(data rate): %d", len);
	...
	/*WISPr*/
	pulRateMinUp = (u_int32_t *)radius_msg_get_vendor_attr(msg, RADIUS_VENDOR_ID_WISPR, RADIUS_VENDOR_ATTR_WISPR_BANDWIDTH_MIN_UP, (size_t *)&len);
	if(pulRateMinUp && len == 4){
		ulRate = ntohl(*pulRateMinUp);
		VM_FREE(pulRateMinUp);
		pulRateMinUp = NULL;

		ModuleLogMsg(APP_MODULE_HOSTAPD, APP_DETAIL, "wispr attr(min up rate): %d", ulRate);
		pstQos->ulUpBandMin = ulRate/1000;
		attr_rate++;
	}
	else if(pulRateMinUp){
		ModuleLogMsg(APP_MODULE_HOSTAPD, APP_ERROR, "error len: wispr attr(min up rate): %d", len);
		
		VM_FREE(pulRateMinUp);
		pulRateMinUp = NULL;
	}

	pulRateMinDown = (u_int32_t *)radius_msg_get_vendor_attr(msg, RADIUS_VENDOR_ID_WISPR, RADIUS_VENDOR_ATTR_WISPR_BANDWIDTH_MIN_DOWN, (size_t *)&len);
	if(pulRateMinDown && len == 4){
		ulRate = ntohl(*pulRateMinDown);
		VM_FREE(pulRateMinDown);
		pulRateMinDown = NULL;

		ModuleLogMsg(APP_MODULE_HOSTAPD, APP_DETAIL, "wispr attr(min down rate): %d", ulRate);
		pstQos->ulDownBandMin = ulRate/1000;
		attr_rate++;
	}
	else if(pulRateMinDown){
		ModuleLogMsg(APP_MODULE_HOSTAPD, APP_ERROR, "error len: wispr attr(min up rate): %d", len);
	...
	pulRateMaxUp = (u_int32_t *)radius_msg_get_vendor_attr(msg, RADIUS_VENDOR_ID_WISPR, RADIUS_VENDOR_ATTR_WISPR_BANDWIDTH_MAX_UP, (size_t *)&len);
	if(pulRateMaxUp && len == 4){
		ulRate = ntohl(*pulRateMaxUp);
		VM_FREE(pulRateMaxUp);
		pulRateMaxUp = NULL;

		ModuleLogMsg(APP_MODULE_HOSTAPD, APP_DETAIL, "wispr attr(min down rate): %d", ulRate);
		pstQos->ulUpBandMax = ulRate/1000;
		attr_rate++;
	}
	else if(pulRateMaxUp){
		ModuleLogMsg(APP_MODULE_HOSTAPD, APP_ERROR, "error len: wispr attr(min up rate): %d", len);
	...
	pulRateMaxDown = (u_int32_t *)radius_msg_get_vendor_attr(msg, RADIUS_VENDOR_ID_WISPR, RADIUS_VENDOR_ATTR_WISPR_BANDWIDTH_MAX_DOWN, (size_t *)&len);
	if(pulRateMaxDown && len == 4){
		ulRate = ntohl(*pulRateMaxDown);
		VM_FREE(pulRateMaxDown);
		pulRateMaxDown = NULL;

		ModuleLogMsg(APP_MODULE_HOSTAPD, APP_DETAIL, "wispr attr(min down rate): %d", ulRate);
		pstQos->ulDownBandMax = ulRate/1000;
		attr_rate++;
	}
	else if(pulRateMaxDown){
		ModuleLogMsg(APP_MODULE_HOSTAPD, APP_ERROR, "error len: wispr attr(min up rate): %d", len);
	...
	if(attr_rate){
		if(pstQos->ulUpBandMax < pstQos->ulUpBandMin)
			pstQos->ulUpBandMax = pstQos->ulUpBandMin;
		if(pstQos->ulDownBandMax < pstQos->ulDownBandMin)
			pstQos->ulDownBandMax = pstQos->ulDownBandMax;
		return;
	}

	/*Mikrotik*/
	pszRateLimit = radius_msg_get_vendor_attr(msg, RADIUS_VENDOR_ID_MIKROTIK, RADIUS_VENDOR_ATTR_MIKROTIK_RATE_LIMIT, &len);
	if(pszRateLimit){
		char *pos_rx_rate = pszRateLimit, *pos_tx_rate = NULL, *pos_end = NULL;
		/* int ret; */
		...
		pos_end = strchr(pos_rx_rate, ' ');
		if(pos_end)
			*pos_end = '\0';

		pos_tx_rate = strchr(pos_rx_rate, '/');
		if(pos_tx_rate){
			*pos_tx_rate = '\0';
			pos_tx_rate++;
		}
		...
		/*读RX_RATE*/
		if(os_strlen(pos_rx_rate)){
			char *pos_end2 = NULL;
			
			if((pos_end2 = strchr(pos_rx_rate, 'k')) != NULL
				|| (pos_end2 = strchr(pos_rx_rate, 'K')) != NULL){
				*pos_end2 = '\0';
				if(sscanf(pos_rx_rate, "%d", &ulRate) == 1)
					pstQos->ulDownBandMax = pstQos->ulDownBandMin = pstQos->ulUpBandMax = pstQos->ulUpBandMin = ulRate;
			}
			else if((pos_end2 = strchr(pos_rx_rate, 'm')) != NULL
				|| (pos_end2 = strchr(pos_rx_rate, 'M')) != NULL){
				*pos_end2 = '\0';
				if(sscanf(pos_rx_rate, "%d", &ulRate) == 1)
					pstQos->ulDownBandMax = pstQos->ulDownBandMin = pstQos->ulUpBandMax = pstQos->ulUpBandMin = 1000*ulRate;
			}
			else{
				if(sscanf(pos_rx_rate, "%d", &ulRate) == 1)
					pstQos->ulDownBandMax = pstQos->ulDownBandMin = pstQos->ulUpBandMax = pstQos->ulUpBandMin = ulRate/1000;
			}
		}

		/*读TX_RATE*/
		if(os_strlen(pos_tx_rate)){
			char *pos_end2 = NULL;
			
			if((pos_end2 = strchr(pos_tx_rate, 'k')) != NULL
				|| (pos_end2 = strchr(pos_tx_rate, 'K')) != NULL){
				*pos_end2 = '\0';
				if(sscanf(pos_tx_rate, "%d", &ulRate) == 1)
					pstQos->ulDownBandMax = pstQos->ulDownBandMin = ulRate;
			}
			else if((pos_end2 = strchr(pos_tx_rate, 'm')) != NULL
				|| (pos_end2 = strchr(pos_tx_rate, 'M')) != NULL){
				*pos_end2 = '\0';
				if(sscanf(pos_tx_rate, "%d", &ulRate) == 1)
					pstQos->ulDownBandMax = pstQos->ulDownBandMin = 1000*ulRate;
			}
			else{
				if(sscanf(pos_tx_rate, "%d", &ulRate) == 1)
					pstQos->ulDownBandMax = pstQos->ulDownBandMin = ulRate/1000;
		...
		VM_FREE(pszRateLimit);
		pszRateLimit = NULL;
		return;
	}

	/*ZTE*/
	pulRateUp = (u_int32_t *)radius_msg_get_vendor_attr(msg, RADIUS_VENDOR_ID_ZTE, RADIUS_VENDOR_ATTR_ZTE_RATE_CTRL_SRC_UP, (size_t *)&len);
	if(pulRateUp && len == 4){
		ulRate = ntohl(*pulRateUp);
		VM_FREE(pulRateUp);
		pulRateUp = NULL;

		ModuleLogMsg(APP_MODULE_HOSTAPD, APP_DETAIL, "zte attr(rate ctrl src up): %d kbps", ulRate);
		pstQos->ulUpBandMin = pstQos->ulUpBandMax = ulRate;

		attr_rate++;
	}
	else if(pulRateUp){
		ModuleLogMsg(APP_MODULE_HOSTAPD, APP_ERROR, "error len: zte attr(rate ctrl src up): %d", len);
		
		VM_FREE(pulRateUp);
		pulRateUp = NULL;
	}
	
	pulRateDown = (u_int32_t *)radius_msg_get_vendor_attr(msg, RADIUS_VENDOR_ID_ZTE, RADIUS_VENDOR_ATTR_ZTE_RATE_CTRL_SRC_DOWN, (size_t *)&len);
	if(pulRateDown && len == 4){
		ulRate = ntohl(*pulRateDown);
		VM_FREE(pulRateDown);
		pulRateDown = NULL;

		ModuleLogMsg(APP_MODULE_HOSTAPD, APP_DETAIL, "zte attr(rate ctrl src down): %d kbps", ulRate);
		pstQos->ulDownBandMin = pstQos->ulDownBandMax = ulRate;
		
		attr_rate++;
	}
	else if(pulRateDown){
		ModuleLogMsg(APP_MODULE_HOSTAPD, APP_ERROR, "error len: zte attr(rate ctrl src down): %d", len);
	...
	/*WinRadius小工具，控制带宽*/
	len = sizeof(class_rate)-1;
	if(radius_msg_get_attr(msg, RADIUS_ATTR_CLASS, class_rate, len) == 32){
		char szRateUpMax[9] = {0}, szRateUpMin[9] = {0}, szRateDownMax[9] = {0}, szRateDownMin[9] = {0};

		ModuleLogMsg(APP_MODULE_HOSTAPD, APP_DETAIL, "class attr: %s", class_rate);
		
		os_memcpy(szRateUpMax, class_rate, 8);
		if(sscanf(szRateUpMax, "%d", &ulRate) == 1)
			pstQos->ulUpBandMax = ulRate/1000;
		
		os_memcpy(szRateUpMin, class_rate+8, 8);
		if(sscanf(szRateUpMin, "%d", &ulRate) == 1)
			pstQos->ulUpBandMin = ulRate/1000;
		
		os_memcpy(szRateDownMax, class_rate+16, 8);
		if(sscanf(szRateDownMax, "%d", &ulRate) == 1)
			pstQos->ulDownBandMax = ulRate/1000;
		
		os_memcpy(szRateDownMin, class_rate+24, 8);
		if(sscanf(szRateDownMin, "%d", &ulRate) == 1)
			pstQos->ulDownBandMin = ulRate/1000;
	...
	/*gbcom*/
	len = radius_msg_get_attr(msg, RADIUS_ATTR_USER_DATA_QOS, (u8 *)& user_info, sizeof(user_info)) ;
	if(len == sizeof(user_info)){
		user_info.ulInPriority = ntohl(user_info.ulInPriority) ;
		user_info.ulInMaxULBand = ntohl(user_info.ulInMaxULBand) ;
		user_info.ulInMaxDLBand = ntohl(user_info.ulInMaxDLBand) ;
		user_info.ulInMinULBand = ntohl(user_info.ulInMinULBand) ;
		user_info.ulInMinDLBand = ntohl(user_info.ulInMinDLBand) ;
		user_info.ulOutPriority = ntohl(user_info.ulOutPriority) ;
		user_info.ulOutMaxULBand = ntohl(user_info.ulOutMaxULBand) ;
		user_info.ulOutMaxDLBand = ntohl(user_info.ulOutMaxDLBand) ;
		user_info.ulOutMinULBand = ntohl(user_info.ulOutMinULBand) ;
		user_info.ulOutMinDLBand = ntohl(user_info.ulOutMinDLBand) ;

		pstQos->ulPriority = user_info.ulOutPriority ;
		pstQos->ulUpBandMax = user_info.ulOutMaxULBand ;
		pstQos->ulUpBandMin = user_info.ulOutMinULBand ;
		pstQos->ulDownBandMax = user_info.ulOutMaxDLBand ;
		pstQos->ulDownBandMin = user_info.ulOutMinDLBand ;
	}
