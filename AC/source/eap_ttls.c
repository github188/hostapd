				    ...
				    int mandatory)
{
	...
	avp = VM_MALLOC(sizeof(struct ttls_avp) + *resp_len + 4);
	if (avp == NULL) {
		VM_FREE(*resp);
		*resp_len = 0;
		return -1;
	...
	while (left > 0) {
		...
		if (vendor_id == 0 && avp_code == RADIUS_ATTR_EAP_MESSAGE) {
			...
			if (parse->eap == NULL) {
				...
				u8 *neweap = VM_REALLOC(parse->eap,
						     parse->eap_len + dlen);
				if (neweap == NULL) {
					wpa_printf(MSG_WARNING, "EAP-TTLS: "
						   "failed to allocate memory "
			     ...
			     size_t *out_len)
{
	...
	req = VM_MALLOC(sizeof(struct eap_hdr) + 2 + data->ssl.tls_out_limit);
	if (req == NULL)
		return NULL;
				    ...
				    u8 *in_data, size_t in_len)
{
	...
	in_decrypted = VM_MALLOC(buf_len);
	if (in_decrypted == NULL) {
		VM_FREE(data->ssl.tls_in);
	...
	if (parse.user_name) {
		VM_FREE(sm->identity);
		sm->identity = VM_MALLOC(parse.user_name_len);
		if (sm->identity) {
