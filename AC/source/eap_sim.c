...
static void eap_sim_reset(struct eap_sm *sm, void *priv)
{
	...
	VM_FREE(data->next_pseudonym);
	VM_FREE(data->next_reauth_id);
	VM_FREE(data);
}
				  ...
				  struct eap_sim_attrs *attr)
{
	...
	if (attr->identity) {
		VM_FREE(sm->identity);
		sm->identity = VM_MALLOC(attr->identity_len);
		if (sm->identity) {
...
static u8 * eap_sim_getKey(struct eap_sm *sm, void *priv, size_t *len)
{
	...
	key = VM_MALLOC(EAP_SIM_KEYING_DATA_LEN);
	if (key == NULL)
		return NULL;
