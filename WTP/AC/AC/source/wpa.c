...
static int wpa_gen_wpa_ie(struct wpa_authenticator *wpa_auth)
{
	...
	VM_FREE(wpa_auth->wpa_ie);
	wpa_auth->wpa_ie = VM_MALLOC(pos - buf);
			       ...
			       const u8 *addr, u16 reason)
{
	if (wpa_auth->cb.disconnect == NULL)
		return;
	wpa_auth->cb.disconnect(wpa_auth->cb.ctx, addr,reason);
}
			...
			const u8 *wpa_ie, size_t wpa_ie_len)
{
...
#if 0 /*由于与TP-LINK终端不兼容，故暂不考察预鉴权机制*/
	...
	if (sm->wpa_ie == NULL || sm->wpa_ie_len < wpa_ie_len) {
		VM_FREE(sm->wpa_ie);
		sm->wpa_ie = VM_MALLOC(wpa_ie_len);
		if (sm->wpa_ie == NULL)
		 ...
		 u8 *data, size_t data_len)
{
	...
	VM_FREE(sm->last_rx_eapol_key);
	sm->last_rx_eapol_key = VM_MALLOC(data_len);
...
SM_STATE(WPA_PTK, DISCONNECT)
{
	...
	sm->Disconnect = FALSE;
	wpa_sta_disconnect(sm->wpa_auth, sm->addr, sm->disconnect_reason?sm->disconnect_reason:WLAN_REASON_PREV_AUTH_NOT_VALID);
}
...
SM_STATE(WPA_PTK, PTKSTART)
{
	...
	sm->PTKRequest = FALSE;
	sm->TimeoutEvt = FALSE;
	((struct hostapd_data *)(sm->wpa_auth->cb.ctx))->stats.wpa_req ++ ;
	wpa_auth_logger(sm->wpa_auth, sm->addr, LOGGER_DEBUG,
...
SM_STATE(WPA_PTK, PTKCALCNEGOTIATING)
{
	...
	if (!ok) {
		...
		if(sm->wpa_auth == NULL
			|| (hapd = sm->wpa_auth->cb.ctx) == NULL){
			wpa_printf(MSG_ERROR, "%s, point is NULL.\n", __func__) ;
			return ;
		}

		os_memcpy(ext.stUser.aucMac, sm->addr, 6) ;
		if(hostapd_report_event(hapd, 1, AC_OMA_EVENT_ID_USER_ACCESS_DENY, AC_OMA_EVENT_REASON_USER_ACCESS_DENY_WPAPSK, & ext) == 0)
			wpa_printf(MSG_MSGDUMP, "%s, report event success[illegal user"MACSTR" access(%d/%d/%d:%s) failed: WPA PSK error].\n", 
				__func__, MAC2STR(sm->addr), hapd->iface->pstWtp->usWtpId, hapd->iface->interface_id, hapd->vap_id, hapd->conf->iface) ;
		else
			wpa_printf(MSG_ERROR, "%s, report event failed[illegal user"MACSTR" access(%d/%d/%d:%s) failed: WPA PSK error].\n", 
				__func__, MAC2STR(sm->addr), hapd->iface->pstWtp->usWtpId, hapd->iface->interface_id, hapd->vap_id, hapd->conf->iface) ;
...
SM_STATE(WPA_PTK, PTKINITDONE)
{
	...
	((struct hostapd_data *)(sm->wpa_auth->cb.ctx))->stats.wpa_success ++ ;
	sm->EAPOLKeyReceived = FALSE;
	if (sm->Pair) {
		char *alg;
		int klen;
