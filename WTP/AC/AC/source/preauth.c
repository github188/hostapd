...
static int rsn_preauth_iface_add(struct hostapd_data *hapd, const char *ifname)
{
	...
	piface->ifname = VM_STRDUP(ifname);
...
int rsn_preauth_iface_init(struct hostapd_data *hapd)
{
	...
	tmp = VM_STRDUP(hapd->conf->rsn_preauth_interfaces);
	if (tmp == NULL)
		return -1;
	start = tmp;
	for (;;) {
		      ...
		      u8 *buf, size_t len)
{
	...
	ethhdr = VM_MALLOC(sizeof(*ethhdr) + len);
	if (ethhdr == NULL)
		return;
	...
	if (l2_packet_transmit(piface, sta->addr, ETH_P_PREAUTH, (u8 *) ethhdr,
			   sizeof(*ethhdr) + len) < 0) {
